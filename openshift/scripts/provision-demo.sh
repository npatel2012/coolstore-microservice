...
# config
GITHUB_ACCOUNT=${GITHUB_ACCOUNT:-npatel2012}
...
function add_service_templates_to_projects() {
  ...
  curl -sL $_TEMPLATE | tr -d '\n' | tr -s '[:space:]' \
    ...
    | sed "s|\"https://github.com/npatel2012/coolstore-microservice\"|\"http://$GOGS_ROUTE/$GOGS_USER/coolstore-microservice.git\"|g" \
